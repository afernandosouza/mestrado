...
class ExperimentGUI:
    ...
    def _plot_bp_space(self, all_texts_data, selected_lang_codes, dim, tau):
        ...
        for lang_code in selected_lang_codes:
            ...
            self.ax.scatter(hs, C, c=color, s=15, alpha=0.6, label=f"{lang_code} (n={len(hs)})", zorder=3) # Zorder para pontos
            self.ax.scatter(centroid[0], centroid[1], c=color, s=100, marker="X", edgecolors='black', linewidths=0.8, label=f"Centroide {lang_code}", zorder=4) # Zorder para centroide
    ...
    def _plot_fs_space(self, all_texts_data, selected_lang_codes, dim, tau):
        ...
        for lang_code in selected_lang_codes:
            ...
            # Plotar os pontos e o centroide para cada idioma
            color = self.color_map[lang_code] # Usa o código para pegar a cor
            self.ax.scatter(hs, F, c=color, s=15, alpha=0.6, label=f"{lang_code} (n={len(hs)})", zorder=3) # Zorder para pontos
            self.ax.scatter(centroid[0], centroid[1], c=color, s=100, marker="X", edgecolors='black', linewidths=0.8, label=f"Centroide {lang_code}", zorder=4) # Zorder para centroide
    ...
    def _compare_text_in_space(self, space: str):
        ...
        # Idioma de referência: primeiro selecionado
        ref_lang = selected_langs[0]

        if ref_lang not in self.current_stats_multi or self.current_stats_multi[ref_lang]["space"] != space:
            messagebox.showerror("Erro", f"Gere primeiro o gráfico {space.upper()} para o idioma de referência '{ref_lang}'.")
            return
        ...
        # Estatísticas do idioma de referência
        ref_stats = self.current_stats_multi[ref_lang]
        ...
        # Calcula Hs e C/F com esses parâmetros
        if space == "bp":
        ...
        # Distância Mahalanobis diagonal
        ...
        msg = (
            ...
            f"dist={dist:.2f} (vs {ref_lang}, limiar={threshold:.2f}). "
