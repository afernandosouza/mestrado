...
def extrair_wavelet_packet_features(df, wavelet='db1', nivel=5):
    ...
    subband_features = np.log1p(np.abs(medianas))

    # Métricas do artigo
    hs, cjs = bandt_pompe_entropy_complexity(dados, D=6)
    return np.concatenate([subband_features, [hs, cjs]])
...
def treinar_mlp_por_cluster(df_wavelet):
    ...
    for cluster_id in sorted(df_wavelet['cluster'].unique()):
        ...
        X = df_cluster[[f'Subbanda_{i}' for i in range(32)] + ['HS', 'CJS']].values
...
def bandt_pompe_entropy_complexity(series, D=6):
    if len(series) < D:
        raise ValueError("Série muito curta para D=" + str(D))
    
    patterns = []
    for i in range(len(series) - D + 1):
        sorted_idx = tuple(np.argsort(series[i:i+D]))
        patterns.append(sorted_idx)

    counts = Counter(patterns)
    total = sum(counts.values())
    P = np.array([counts[p] / total for p in permutations(range(D))])
    P = P[P > 0]  # eliminar zeros
    
    HS = entropy(P) / np.log(factorial(D))  # entropia de Shannon normalizada

    Pe = np.full(len(P), 1.0 / len(P))
    M = 0.5 * (P + Pe)
    QJ = entropy(M) - 0.5 * (entropy(P) + entropy(Pe))  # divergência de Jensen-Shannon
    Q0 = -0.5 * ((1 + len(P)) * np.log(1 + len(P)) - 2 * len(P) * np.log(2 * len(P)) + len(P) * np.log(len(P)))
    CJS = QJ * HS / Q0 if Q0 != 0 else 0
                ...
                for idx, row in enumerate(df_cluster.itertuples(index=False)):
                    features.append({
                        ...
                        **{f'Subbanda_{i}': wavelet_features[i] for i in range(32)},
                        'HS': wavelet_features[32],
                        'CJS': wavelet_features[33],
                    })
            except Exception as e:
